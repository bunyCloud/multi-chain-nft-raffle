...
contract DutchSwapFactory is Owned, CloneFactory {
  ...
  event AuctionTemplateUpdated(
    address oldDutchAuction,
    address newDutchAuction
  );

  function initDutchSwapFactory(
    address _dutchAuctionTemplate,
    uint256 _minimumFee
  ) public {
  ...
  function deployDutchAuction(
    ...
    require(
      IERC20(_token).transferFrom(msg.sender, address(this), _tokenSupply)
    );
