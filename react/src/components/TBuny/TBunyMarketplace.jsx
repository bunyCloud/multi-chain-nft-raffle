...
import NFTCard from "./TBunyNftCard";
import {
  TBunyNftAddress,
...
export default function TBunyMarketplace(trigger) {
  const [nfts, setNfts] = useState([]);
  const [loaded, setLoaded] = useState(false);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    console.log({ TBunyNftAddress, TBunyMarketplaceAddress });
    loadNfts();
  }, []);
  ...
  async function loadNfts() {
    ...
    const nft = new ethers.Contract(TBunyNftAddress, NFT.abi, provider);
  ...
  async function buyNft(nft) {
    ...
    const transaction = await marketplace.buyItem(TBunyNftAddress, nft.itemId, {
      value: price,
    });
